...
const DateFilterNocoDB = () => {
  ...
  //       whereClause += whereClause ? `~and(orders_2.channel,eq,${channel})` : `(orders_2.channel,eq,${channel})`;
  //     }
  ...
  //       setError(`No records found for the selected filters. Query used: ${whereClause}`);
  //     }
  //   } catch (error) {
...
const fetchLocation = async () => {
  ...
  const filterData = () => {
    ...
    if (employeeSearch) {
      result = result.filter(item => 
        item.employees?.user_name?.toLowerCase().includes(employeeSearch.toLowerCase())
      );
    }
  ...
  const getStats = () => {
    ...
    // Count by order_id
    const orderCount = new Set(filteredData.map(item => item.order_id)).size;
    
    // Count by employee
    const employeeCounts = filteredData.reduce((acc, item) => {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gray-800">Order Tracking Dashboard</h1>
            <p className="text-gray-600">Monitor and analyze order processing activities</p>
          </div>
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="bg-white p-6 rounded-xl shadow">
                <h3 className="font-medium text-gray-700 mb-4">Total Orders Processed</h3>
                <p className="text-4xl font-bold text-blue-600">{stats.orderCount}</p>
                <p className="text-sm text-gray-500 mt-2">Unique order IDs scanned</p>
              </div>
              
              <div className="bg-white p-6 rounded-xl shadow">
                <h3 className="font-medium text-gray-700 mb-4">Top Employees</h3>
                    ...
                    .map(([employee, count]) => (
                      <div key={employee} className="flex items-center">
                        <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center mr-3">
                          <span className="text-blue-600 text-sm font-medium">{employee.charAt(0)}</span>
                        </div>
                        <div className="flex-1">
                          <div className="flex justify-between text-sm">
                            <span className="font-medium">{employee.split(" / ")[0]} </span>
                            <span className="font-bold">{count}</span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-1.5 mt-1">
                            <div 
                              className="bg-blue-600 h-1.5 rounded-full" 
                              style={{ width: `${(count / Math.max(...Object.values(stats.employeeCounts)) * 100)}%` }}
                            ></div>
                          </div>
                        </div>
                      </div>
              ...
              <div className="bg-white p-6 rounded-xl shadow">
                <h3 className="font-medium text-gray-700 mb-4">Top Locations</h3>
                <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
              <div className="bg-white p-6 rounded-xl shadow">
                <h3 className="font-medium text-gray-700 mb-4">Employee Activity</h3>
                <div className="h-80">
              ...
              <div className="bg-white p-6 rounded-xl shadow">
                <h3 className="font-medium text-gray-700 mb-4">Location Distribution</h3>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Scan Time</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredData.map((record, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">{record.order_id}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{record.orders_2?.style_number || "N/A"}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{record.orders_2?.channel || "N/A"}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex items-center">
                          <div className="flex-shrink-0 h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center mr-2">
                            <span className="text-blue-600 text-xs font-medium">{record.employees?.user_name?.charAt(0) || "?"}</span>
                          </div>
                          <div>{record.employees?.user_name.split(" / ")[0] || "N/A"}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {record.scanned_timestamp ? new Date(record.scanned_timestamp).toLocaleString() : "N/A"}
                      </td>
                    </tr>
                  ))}
                </tbody>
